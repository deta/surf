...
我们构建了一个用户界面，可以在每次查询时切换模型。我们设计了 Surf 的架构，使您可以为流行模型提供自己的密钥，或者添加自己的自定义模型（本地或云端）。
...
Surf 的界面允许您随时轻松切换模型，使用最适合您需求的模型。只需在切换器中选择即可。
...
如果您尚未设置自己的 AI 模型，请按照以下一些 [**常见提供商**](#common-providers) 的说明操作。一旦您的模型运行起来，您可以按照以下步骤将其添加到 Surf 中：

1. Open the Surf settings through the app menu or using the shortcut `⌘/ctrl + ,`
1. 通过应用程序菜单或使用快捷键 `⌘/ctrl + ,` 打开 Surf 设置
...
添加模型后，您可以开始在 Surf 中使用它。从设置页面顶部的下拉菜单或从 Chat 或 Smart Notes 编辑器中选择它。
...
要使用由 [OpenRouter](https://openrouter.ai/) 托管的模型，请首先创建一个账户，然后从他们的 [设置页面](https://openrouter.ai/settings/keys) 生成 API 密钥。使用以下配置选项将其连接到 Surf：
...
1. Add a new model in Surf's AI settings and configure it like below:
1. 在 Surf 的 AI 设置中添加新模型，并按照以下方式配置：
...
您也可以在 Hugging Face 上部署自己的 [推理端点](https://endpoints.huggingface.co/) 并将其与 Surf 一起使用。首先查看他们的 [关于如何创建兼容 OpenAI 的推理端点的指南](https://huggingface.co/blog/tgi-messages-api)，然后在 Surf 的 AI 设置中添加新模型，并按照以下方式配置：
...
理论上，任何提供 OpenAI 兼容的聊天补全 API 的提供商都应该能够与 Surf 一起工作。但实际上，一些提供商可能不完全兼容，并且可能有其自身的特性。确定特定提供商是否工作的唯一方法是尝试使用它。请参考上面的 [配置选项](https://www.notion.so/Configuration-Options-199a5244a717800081a7ce12cef7bd93?pvs=21) 了解所需信息。
...
您可以从 [Deta 网站](https://deta.surf/) 下载带有托管 AI 的 Surf 版本。
