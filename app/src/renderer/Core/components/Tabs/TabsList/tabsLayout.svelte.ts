...
export function calculateTabLayout(
  ...
  const availableWidth =
    containerWidth - cfg.containerPadding - addButtonWidth - gapsWidth;
  ...
  if (activeIndices.length > 0) {
    ...
    for (const idx of activeIndices) {
      ...
      const width = Math.max(
        ...
        Math.min(
          cfg.maxTabWidth,
          Math.min(cfg.activeTabMinWidth, perActiveTarget),
        ),
      );
  ...
  return {
    ...
    addButtonWidth,
    totalWidth:
      cfg.containerPadding + totalTabsWidth + gapsWidth + addButtonWidth,
  };
}
...
function uniformLayout(
  ...
  return {
    ...
    addButtonWidth,
    totalWidth:
      DEFAULT_CONFIG.containerPadding +
      tabWidth * count +
      gapsWidth +
      addButtonWidth,
  };
