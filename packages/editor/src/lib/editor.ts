...
import mentionSuggestion, {
  type MentionItemsFetcher,
} from "./extensions/Mention/suggestion";
import {
  CaretIndicatorExtension,
  type CaretPosition,
} from "./extensions/CaretIndicator";
import Loading from "./extensions/Loading";
import Thinking from "./extensions/Thinking";
import TrailingNode, {
  EnsureTrailingParagraph,
} from "./extensions/TrailingNode";
...
export const createEditorExtensions = (opts?: ExtensionOptions) => [
  ...
  ...conditionalArrayItem(
    !!opts?.enableCaretIndicator &&
      opts?.onFloatyInputStateChange !== undefined,
    [
    ...
    addProseMirrorPlugins() {
      const plugin = new Plugin({
        ...
        props: {
          handleDOMEvents: {
            paste(_, e) {
              const clipboardDataItems = Array.from(
