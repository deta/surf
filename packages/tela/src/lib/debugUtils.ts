...
function clipboard(options = {}) {
  ...
  async function update_text() {
    ...
    if (unstore(clipboard_supported)) {
      const value =
        (_a = await (navigator == null ? void 0 : navigator.clipboard.readText())) != null ? _a : ''
      text.set(value)
...
function network() {
  ...
  if (unstore(supported2)) eventListener(navigator.connection, 'change', update_network_info, false)
