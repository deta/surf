<script lang="ts">
  ...
  import DragDropList, { HorizontalCenterDropZone } from 'svelte-dnd-list'
  ...
  const log = useLogScope('MiniOasis')
  ...
  const pinnedItems = writable<Resource[]>([])
  ...
  const loadEverything = async () => {
    try {
      ...
      pinnedItems.set(
        resources.filter((x) =>
          x.tags?.some((tag) => tag.value === 'true' && tag.name === 'pinned')
        )
      )
...
<div class="flex flex-col gap-1 h-full overflow-y-scroll">
  ...
  <DropWrapper
    on:drop={async (e) => {
      ...
      await resourceManager.updateResourceTag(parsed[0].data, 'pinned', 'true')

      await loadSpaceContents(spaceId)
      toasts.success('Resources added!')
    }}
  >
    <!-- <div class="w-full h-24 bg-red-100" /> -->
    <DragDropList
      id="pinned-tabs"
      zoneClass="flex items-center space-x-2 w-full h-48"
      type={HorizontalCenterDropZone}
      itemSize={100}
      itemCount={$pinnedItems.length || 1}
      on:drop={async (e) => {
        const event = e.detail
        log.debug('Dropped', event)
    ...
    >
      ...
      {:else}
        {#key $pinnedItems[index]}
          <!-- <ResourcePreviewClean
            resource={$pinnedItems[index]}
            size="small"
