...
<script lang="ts">
  ...
  const stackTransformY = derived(
    [showOverview, scaling, activeIdx, overviewOffset, movementOffset],
    ([showOverview, scaling, activeIdx, overviewOffset, movementOffset]) => {
...
<div
  ...
  class:transitions={showTransitions}
  style="--transition-duration: {opts.transitionDuration /
    1000}s; --transition-timing-function: {opts.transitionTimingFunction}; --scaling: {$scalingTweened}; --padding-top: {$paddingTop}px; --down-scaled: {opts.scaling}; --gap: {opts.gap}px;"
>
  <div
    class="list"
    style="--stack-y: {showTransitions ? $tweenedStackTransformY : $stackTransformY}px;"
  >
    <slot />
  </div>
</div>
