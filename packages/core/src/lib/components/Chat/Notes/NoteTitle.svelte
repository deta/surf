<script lang="ts">
  ...
  const handleUpdateTitle = useDebounce((e) => {
    ...
    note.updateTitle(value)
  }, 500)

  const handleBlur = async (e) => {
    const value = e.target.value
    if (!value) {
      const generatedTitle = await note.generateTitle()
      if (generatedTitle) {
...
<div class="chat-title" class:small class:reveal={showReveal}>
  <input
    ...
    placeholder={$generatingTitle ? 'Generating title…' : 'Note title'}
...
<style lang="scss">
  .chat-title {
    ...
    :global(.dark) & {
      --squircle-fill: rgba(255, 255, 255, 0.12);
