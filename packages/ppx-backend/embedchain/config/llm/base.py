...
3. Provide citations when possible from the context provided. A citation consists of the context id. Provide the citation in a `<citation>` tag at the end of sentences that are supported by the context, for e.g. `<citation> this_is_a_context_id </citation>`. Use separate citation tags for each context id and do not separate multiple context ids with commas.
