...
const cleanContent = (text: string, hostname: string | null) => {
  ...
  if (hostname === "github.com") {
    const regex =
      /Contribute to ([\w-]+\/[\w-]+) development by creating an account on GitHub\./;
...
export const getResourcePreview = async (
  ...
  const canonicalUrl = parseStringIntoUrl(
    resource.tags?.find((x) => x.name === ResourceTagsBuiltInKeys.CANONICAL_URL)
      ?.value ||
  ...
  const userMediaResource = previewImageId
    ? `surf://resource/${previewImageId}`
    : null;
  ...
  try {
    if (resource instanceof ResourceJSON) {
      ...
      if (resource.type === ResourceTypes.LINK) {
        ...
        const hideContent = HIDE_CONTENT_FOR_SITES.some(
          (site) => hostname === site,
        );
        ...
        if (!options.showAnnotations && annotations.length > 0) {
          ...
          const comment = (annotationData.data as AnnotationCommentData)
            .content_plain;
          const highlight = (annotationData.anchor?.data as AnnotationRangeData)
            .content_plain;
          if (comment) {
          ...
          annotations
            .slice(1)
            .forEach(() =>
              annotationItems.push({ type: "highlight", content: "" }),
            );
        }
        ...
        const resourceContent = cleanContent(
          data.excerpt || data.content_plain,
          hostname,
        );
        const previewContent = summary || resourceContent || undefined;
        ...
        previewData = {
          ...
          title:
            (resource?.metadata?.name &&
              resource?.metadata?.name !== resourceContent) ||
        ...
        const commentContent = (data?.data as AnnotationCommentData)
          .content_plain;
        const highlightContent = (data.anchor?.data as AnnotationRangeData)
          .content_plain;

        const source =
          data?.type === "comment"
            ? (data.data as AnnotationCommentData).source
            : null;
        const sourceClean =
          source === "inline_ai"
            ? `Inline AI`
        ...
        previewData = {
          ...
          annotations: highlightContent
            ? [{ type: "highlight", content: highlightContent }]
            : [],
      ...
      previewData = {
        ...
        url:
          canonicalUrl ??
          parseStringIntoUrl(resource.metadata?.sourceURI ?? "")?.href ??
          "",
        source: {
          text:
            resource?.metadata?.name ||
...
export const isGeneratedResource = (resource: Resource) => {
  return (
    (resource.tags ?? []).find(
      (x) =>
        x.name === ResourceTagsBuiltInKeys.SAVED_WITH_ACTION &&
        x.value === "generated",
    ) !== undefined
  );
};
