...
export class Resource extends EventEmitterBase<ResourceEvents> {
  ...
  get url() {
    return parseStringIntoUrl(
      this.tags?.find((x) => x.name === ResourceTagsBuiltInKeys.CANONICAL_URL)
        ?.value ||
...
export class ResourceManager extends EventEmitterBase<ResourceManagerEvents> {
  ...
  private findOrCreateResourceObject(resource: SFFSResource) {
    const existingResource = get(this.resources).find(
      (r) => r.id === resource.id,
    );
    if (existingResource) {
  ...
  private findOrGetResourceObject(
    id: string,
  ...
  private handleEventBusMessage(event: EventBusMessage) {
    if (event.type === EventBusMessageType.ResourceProcessingMessage) {
      this.handleResourceProcessingMessage(
        event.resource_id,
        event.status.type,
      );
    }
  ...
  private async handleResourceProcessingMessage(
    id: string,
    status: ResourceProcessingStateType,
  ) {
    ...
    this.resources.update((resources) =>
      resources.map((r) => (r.id === id ? resource : r)),
    );
  }
  ...
  async createResource(
    ...
    const canonicalUrlTag = tags?.find(
    ...
    const isSilent =
      tags?.find((t) => t.name === ResourceTagsBuiltInKeys.SILENT)?.value ===
      "true";
    const isFromSpace = !!tags?.find(
      (t) => t.name === ResourceTagsBuiltInKeys.SPACE_SOURCE,
    )?.value;
  ...
  async readResources() {
    ...
    const resources = resourceItems.map(
      (item) => new Resource(this.sffs, this, item),
    );
    ...
    const resources = rawResults.items.map(
      (item) =>
        ({
          ...
          annotations: item.resource.annotations?.map((a) =>
  ...
  addAnnotationToLoadedResource(
    ...
    if (loadedResource) {
      this.log.debug(
        "adding annotation to loaded resource",
        resourceId,
        annotation,
      );
      loadedResource.annotations = [
        ...(loadedResource.annotations ?? []),
        annotation,
      ];
    } else {
      this.log.debug(
        "resource not loaded, skipping adding annotation",
        resourceId,
      );
    }
  }
  ...
  async updateResourceMetadata(
    id: string,
    updates: Partial<SFFSResourceMetadata>,
  ) {
    const resource = await this.getResource(id);
    if (!resource) {
      throw new Error("resource not found");
    ...
    const fullMetadata = Object.assign(
      resource.metadata ?? {},
      updates,
    ) as SFFSResourceMetadata;
  ...
  async updateResourceTag(
    ...
    this.resources.update((resources) =>
      resources.map((r) => (r.id === resourceId ? resource : r)),
    );
  }
  ...
  async createResourceTag(
    ...
    const tag = await this.sffs.createResourceTag(
      resourceId,
      tagName,
      tagValue,
    );
  ...
  async preventHiddenResourceFromAutodeletion(
    resourceOrId: ResourceObject | string,
  ) {
    const resource =
      typeof resourceOrId === "string"
        ? await this.getResource(resourceOrId)
        : resourceOrId;
    if (!resource) {
      throw new Error("resource not found");
    }
  ...
  async createDetectedResource<T>(
    ...
    const sourcePublishedAt = (data as any).date_published as
      | string
      | undefined;
    const additionalTags =
      sourcePublishedAt &&
      tags?.findIndex(
        (t) => t.name === ResourceTagsBuiltInKeys.SOURCE_PUBLISHED_AT,
      ) === -1
        ? [ResourceTag.sourcePublishedAt(sourcePublishedAt)]
        : [];
    const allTags = [...(tags ?? []), ...additionalTags];

    return this.createResource(type, blob, metadata, allTags) as Promise<
      ResourceJSON<T>
    >;
  }
  ...
  async searchChatResourcesAI(
    ...
    const rawResources = await this.sffs.searchChatResourcesAI(
      query,
      model,
      opts,
    );
    const resources = rawResources.map((r) =>
      this.findOrCreateResourceObject(r),
    );
    return resources;
  }
  ...
  async deleteSpaceEntries(entries: SpaceEntry[]) {
    ...
    entries.map(async (entry) => {
      ...
      if (resource) {
        this.log.debug(
          "updating resource spaceIds",
          resource.id,
          resource.spaceIdsValue,
        );
        resource.spaceIds.update((ids) =>
          ids.filter((id) => id !== entry.space_id),
        );
        await tick();
        this.log.debug(
          "updated resource spaceIds",
          resource.id,
          resource.spaceIdsValue,
        );
      }
  ...
  async getNumberOfReferencesInSpaces(resourceId: string): Promise<number> {
    ...
    for (const folder of allFolders) {
      ...
      const references = folderContents.filter(
        (content) => content.entry_id === resourceId,
      );
  ...
  async refreshResourceData(resourceOrId: ResourceObject | string) {
    ...
    const canBeRefreshed = WEB_RESOURCE_TYPES.some((x) =>
      resource.type.startsWith(x),
    );
    ...
    if (!canBeRefreshed && !canBeReprocessed) {
      this.log.debug(
        "skipping refresh for non-refreshable resource",
        resource.id,
      );
      return;
    }
    ...
    if (resource.stateValue === "extracting") {
      this.log.debug(
        "skipping refresh as resource is already refreshing",
        resource.id,
      );
      return;
    }
    ...
    try {
      if (canBeReprocessed) {
        ...
        if (
          (resource.tags ?? []).find(
            (x) => x.name === ResourceTagsBuiltInKeys.DATA_STATE,
          )
        ) {
      ...
      const canonicalUrl = parseStringIntoUrl(
        resource.tags?.find(
          (x) => x.name === ResourceTagsBuiltInKeys.CANONICAL_URL,
        )?.value ||
      ...
      if (!canonicalUrl) {
        this.log.debug(
          "skipping refresh as resource has no canonical URL",
          resource.id,
        );
        return;
      }
      ...
      const detectedResource =
        await webParser.extractResourceUsingWebview(document);

      this.log.debug("extracted resource data", detectedResource);

      if (detectedResource) {
        this.log.debug(
          "updating resource with fresh data",
  ...
  static use() {
    if (!ResourceManager.self)
      return getContext<ResourceManager>("resourceManager");
...
class ResourceDebugger {
  ...
  private saveToStorage() {
    try {
      ...
      while (
        data.length > ResourceDebugger.MAX_STORAGE_SIZE &&
  ...
  compare(idx1: number, idx2: number, standalone = true) {
    ...
    if (missing.length > 0) {
      ...
      console.log(
        `Resources: ${s1.resources.length} -> ${s2.resources.length}`,
      );
      console.log("Missing:");
...
export function initResourceDebugger(resourceManager: ResourceManager) {
  ...
  window.debug.help = () => {
    ...
    console.log(
      "debug.resources.compare(i,j)          - Compare two snapshots",
    );
    console.log(
      "debug.resources.sequentialCompare(i)  - Compare snapshots sequentially from index i",
    );
    console.log(
      "debug.resources.compareWithCurrent(i) - Compare snapshot with current state",
    );
