...
export class HistoryEntriesManager {
  ...
  async init() {
    ...
    console.log(
      "HistoryEntriesManager initialized with",
      allEntries.length,
      "entries",
    );
  }
  ...
  private scoreEntry(entry: HistoryEntry, query: string): number {
    ...
    const scorePart = (queryPart: string) => {
      ...
      if (
        entry.searchQuery &&
        entry.searchQuery.toLowerCase().includes(queryPart)
      ) {
        score += 0.25;
      }
  ...
  searchEntries(query: string, threshold: number = 0.2) {
    ...
    for (const entry of entries) {
      if (entry.url)
        urlCount.set(entry.url, (urlCount.get(entry.url) || 0) + 1);
    }
  ...
  searchEntriesFuse(
    ...
    const filteredEntries = fuzzyResults.filter(
      (result: FuseResult<HistoryEntry>) => {
        const dedupKey =
