...
export class ShortcutManager<T extends string> {
  ...
  getShortcutCombo(action: T): string {
    return (
      this.customShortcuts.get(action) ??
      this.defaultShortcuts[action].defaultCombo
    );
  }
  ...
  getShortcutDefinition(
    action: T,
  ): ShortcutDefinition<T> & { currentCombo: string } {
    return {
...
export function useShortcutsManager<T extends string>(): ShortcutManager<T> {
  if (!instance) {
    throw new Error(
      "ShortcutManager not initialized. Please call createShortcutManager first.",
    );
  }
  return instance as ShortcutManager<T>;
}
