...
export class AIService {
  ...
  constructor(resourceManager: ResourceManager, config: ConfigService) {
    ...
    this.alwaysIncludeScreenshotInChat = derived(
  ...
  getMatchingModel(tier: ModelTiers) {
    ...
    if (
      ...
      selectedModel.tier === ModelTiers.Premium
    ) {
      return (
        this.modelsValue.find(
          (m) =>
            m.provider === selectedModel.provider &&
            m.tier === ModelTiers.Standard,
        ) || selectedModel
      );
    }
    ...
    try {
      ...
      if (e instanceof QuotaDepletedError) {
        ...
        if (
          ...
          const tier =
            options.tier === ModelTiers.Standard
              ? ModelTiers.Premium
              : ModelTiers.Standard;
  ...
  async generatePrompts(
    ...
    const prompts = JSON.parse(
      completion.output.replace("```json", "").replace("```", ""),
    );
    ...
    try {
      ...
      if (e instanceof QuotaDepletedError) {
        ...
        if (
          ...
          return this.getResourcesViaPrompt(
            prompt,
            { ...opts, tier: ModelTiers.Standard },
            true,
          );
        }
      }
