...
export class EditorAIGeneration {
  ...
  createNode(loadingMsg?: string) {
    ...
    const generationNode = schema.nodes.generation.create(
      { id: this.id, status: "initializing" },
      [
  ...
  getNodeByType(type: string) {
    ...
    this.log.debug(
      "Node not found directly, trying via parent generation node",
      this.id,
      type,
    );
    ...
    const matchingNodePos =
      nodeWithPos &&
      (nodes ?? []).find((nodePos) => nodePos.node === nodeWithPos.node);
    if (matchingNodePos) {
      this.log.debug(
        "Matching node position found",
        this.id,
        type,
        matchingNodePos,
      );
  ...
  updateOutput(content: string) {
    ...
    if (!this.noteEditor.component) {
      this.log.warn(
        "Note editor component not found, cannot update output",
        this.id,
      );
      return;
    }
    ...
    if (outputNode) {
      ...
      tr.replaceWith(
        outputNode.pos,
        outputNode.pos + outputNode.size,
        newOutputNode,
      );
    } else {
...
export class NoteEditor {
  ...
  getNodeByID(id: string, type: string) {
    ...
    return (
      nodePositions.find((nodePos) => nodePos.node.attrs.id === id) ?? null
    );
  }
