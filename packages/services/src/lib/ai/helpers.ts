...
export const parseXML = (xml: string) => {
  ...
  if (parseError.length > 0) {
    console.warn(
      "Error parsing chat response: " +
        (parseError[0]?.textContent ?? "unknown error"),
    );
    return xmlDoc;
  }
...
export const parseXMLChatResponseSources = (xml: Document) => {
  ...
  const sourceData = Array.from(sources).map((source) => {
    ...
    const resource_id =
      source.getElementsByTagName("resource_id")[0]?.textContent;
...
export const handleInlineAI = async (
  ...
  const content = WebParser.getResourceContent(
...
export const handleQuotaDepletedError = (e: QuotaDepletedError) => {
  ...
  const premiumQuotas = e.quotas.filter(
    (quota) => quota.tier === ModelTiers.Premium,
  );
  const standardQuotas = e.quotas.filter(
    (quota) => quota.tier === ModelTiers.Standard,
  );

  const premiumMonthlyUsed = premiumQuotas.find((quota) =>
    quota.usage_type.startsWith("monthly"),
  );
  const standardMonthlyUsed = standardQuotas.find((quota) =>
    quota.usage_type.startsWith("monthly"),
  );

  const premiumCycleString = premiumMonthlyUsed ? "monthly" : "daily";
  const standardCycleString = standardMonthlyUsed ? "monthly" : "daily";
  const bothCyleString =
    premiumMonthlyUsed && standardMonthlyUsed ? "monthly" : "daily";
...
export const parseAIError = (e: any) => {
  ...
  if (e instanceof TooManyRequestsError) {
    ...
    content =
      "The query failed our content policy checks. Please try again with a different query.";
  } else {
    content = "Failed to generate response.";
  }
  if (
    typeof e === "string" &&
    e.toLowerCase().includes("Content is too long".toLowerCase())
  ) {
    content =
      "The content is too long to process. Please try a more specific question.";
  }
...
const aggregateTextNodes = (
  node: Node,
  text: string,
  stopCitationId: string,
) => {
...
export const mapCitationsToText = (content: HTMLElement) => {
  ...
  const mapCitationsToTextRecursive = (
    node: Node,
    citationsToText: Map<string, string>,
  ) => {
      ...
      if (elem.tagName === "CITATION") {
        ...
        if (text) {
          if (citationsToText.has(citationID)) {
            citationsToText.set(
              citationID,
              citationsToText.get(citationID) + " | " + text,
            );
          } else {
...
export const populateRenderAndChunkIds = (
  sources: AIChatMessageSource[] | undefined,
) => {
  if (!sources) return;
...
export const parseChatOutputToSurfletCode = async (
  output: AIChatMessageParsed,
) => {
  const content = output.content;
...
export const getPrepPhrase = () => {
  return prepLoadingPhrases[
    Math.floor(Math.random() * prepLoadingPhrases.length)
  ];
};

export const getWritingPhrase = () => {
  return writingLoadingPhrases[
    Math.floor(Math.random() * writingLoadingPhrases.length)
  ];
};
