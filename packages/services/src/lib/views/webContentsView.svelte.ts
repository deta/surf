...
export class WebContents extends EventEmitterBase<WebContentsEmitterEvents> {
  ...
  private handlePageTitleUpdated(
    ...
    if (this.view.currentHistoryEntryValue) {
      this.historyEntriesManager.updateEntry(
        this.view.currentHistoryEntryValue.id,
        {
          title: newTitle,
        },
      );
  ...
  private async handlePageFaviconUpdated(
    ...
    const bestFavicon = processFavicons(
      this._lastReceivedFavicons,
      domain,
      isDarkMode,
    );
  ...
  private handleDidNavigate(
    event: WebContentsViewEvents[WebContentsViewEventType.DID_NAVIGATE],
  ) {
  ...
  private handlePreloadIPCEvent(
    ...
    const eventData = event
      .args[1] as WebViewSendEvents[keyof WebViewSendEvents];
    ...
    const unsubWebContentsEvents = window.preloadEvents.onWebContentsViewEvent(
      (event: WebContentsViewEvent) => {
        ...
        if (matchingListeners.length > 0) {
          this.log.debug(
            "Found matching listeners for event",
            event.type,
        ...
        if (event.type === WebContentsViewEventType.DID_START_LOADING) {
          ...
          event.type === WebContentsViewEventType.PAGE_FAVICON_UPDATED
        ) {
          ...
          event.type === WebContentsViewEventType.DID_NAVIGATE_IN_PAGE
        ) {
          this.handleDidNavigateInPage(event.payload);
        } else if (
          event.type === WebContentsViewEventType.MEDIA_STARTED_PLAYING
        ) {
          ...
          event.type === WebContentsViewEventType.ENTER_HTML_FULL_SCREEN
        ) {
          this.handleHtmlFullScreenChange(true);
        } else if (
          event.type === WebContentsViewEventType.LEAVE_HTML_FULL_SCREEN
        ) {
  ...
  constructor(data: WebContentsViewData, manager: ViewManager) {
    ...
    this.currentHistoryEntry = derived(
      ...
      ([historyStackIds, currentHistoryIndex, navigationHistory]) => {
        ...
        return this.historyEntriesManager.getEntry(
