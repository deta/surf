      ...
      main: {
        ...
        sendToWebContents: (webContents: Electron.WebContents, payload: T) => {
          ...
          if (mainProcess) {
            ...
            throw new Error(
              "Cannot send events to web contents in renderer process",
            );
          }
        },
      },
      ...
      renderer: {
        invoke: (payload: T["payload"]) => {
          ...
          if (mainProcess) {
            ...
            return ipcRenderer.invoke(name, payload) as Promise<
              T["output"] | null
            >;
          }
        },
  ...
  registerEvents<T extends { [key: string]: any }>(events: T) {
    ...
    const rendererEvents = Object.keys(events).reduce(
      (acc, key) => {
        ...
        acc[key as keyof typeof events] =
          events[key as keyof typeof events].renderer;
        return acc;
      },
      {} as {
    ...
    const mainEvents = Object.keys(events).reduce(
      (acc, key) => {
        // @ts-ignore
        acc[key as keyof typeof events] =
          events[key as keyof typeof events].main;
        return acc;
      },
      {} as {
