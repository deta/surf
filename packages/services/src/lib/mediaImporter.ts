  ...
  for (const type of DATA_TYPES) {
    ...
    switch (type) {
      ...
      case "text/uri-list":
        ...
        results.push(
          ...urls.map(
            (url) => ({ data: url, type: "url" }) as MediaParserResult,
          ),
        );
        break;
      case "text/tiptap":
...
export const createResourcesFromMediaItems = async (
  ...
  const resources = await Promise.all(
    parsed.map(async (item) => {
      ...
      let resource;
      if (item.type === "text") {
        resource = await resourceManager.createResourceNote(
          item.data,
          item.metadata,
          tags,
        );
...
export const extractAndCreateWebResource = async (
  ...
  const content = WebParser.getResourceContent(
    extractedResource.type,
    extractedResource.data,
  );
...
export const createResourcesFromFiles = async (
  files: File[],
  resourceManager: ResourceManager,
) => {
