...
export class Telemetry {
  ...
  async init(
    ...
    if (this.proxyUrl) {
      ...
      if (!this.userConfig.api_key) {
        this.log.warn(
          "No user API key found for telemetry proxy, disabling telemetry",
        );
        this.active = false;
        return;
      }
    ...
    window.api.onTrackEvent((eventName, properties) => {
      this.log.debug(
        "Received track event from main process",
        eventName,
        properties,
      );
  ...
  async trackEvent(
    ...
    if (!eventName) {
      this.log.warn(
        "No event name provided, not tracking event",
        eventProperties,
      );
      return;
    }
    ...
    let user_properties: UserProperties = {
      ...
      tabs_orientation:
        userSettings?.tabs_orientation ??
        this.userConfig?.settings.tabs_orientation,
      default_browser: this.userConfig?.defaultBrowser,
      extensions_enabled: true, // userSettings?.extensions ?? false,
      notes_sidebar_enabled:
        userSettings?.experimental_notes_chat_sidebar ?? false,
  ...
  async trackSearchOasis(
    trigger: SearchOasisEventTrigger,
    searchingSpace: boolean,
  ) {
  ...
  async trackOpenResource(
    type: string,
    from: OpenResourceEventFrom = OpenResourceEventFrom.Oasis,
  ) {
  ...
  async trackUseInlineAI(
    prompt: InlineAIEventPromptType,
  ...
  async trackPageChatCitationClick(
    type: "timestamp" | "text",
    sourceTabType: "page" | "space",
  ) {
  ...
  async trackPageChatCitationClickResourceFromSpace(
    type: "timestamp" | "text",
  ) {
    await this.trackEvent(
      TelemetryEventTypes.PageChatCitationClickResourceFromSpace,
      {
        type: type,
      },
    );
  ...
  async trackAskInlineAI(data: {
    isFollowUp: boolean;
    baseMedia: AIMessageBaseMedia;
  }) {
  ...
  async trackSaveAIResponse(data: {
