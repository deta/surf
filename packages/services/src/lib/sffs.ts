...
export class SFFS {
  ...
  convertCompositeResourceToResource(
    composite: SFFSRawCompositeResource,
  ): SFFSResource {
    return {
  ...
  convertResourceToCompositeResource(
  ...
  convertRawHistoryEntryToHistoryEntry(
    rawEntry: SFFSRawHistoryEntry,
  ): HistoryEntry {
    return {
      id: rawEntry.id,
  ...
  convertHistoryEntryToRawHistoryEntry(
    entry: HistoryEntry,
  ): SFFSRawHistoryEntry {
    return {
      id: entry.id,
  ...
  convertRawBookmarkFolderToBookmarkFolder(
    rawEntry: SFFSRawBookmarkFolder,
  ): BookmarkFolder {
    return {
      guid: rawEntry.guid,
  ...
  convertRawChatMessageToChatMessage(
    raw: AIChatMessageRaw,
    idx?: number,
  ): AIChatMessage {
    return {
      ...raw,
  ...
  async createResource(
    ...
    const dataString = await this.backend.js__store_create_resource(
      type,
      tagsData,
      metadataData,
    );
  ...
  async updateResourceMetadata(id: string, metadata: SFFSResourceMetadata) {
    ...
    if (!composite || !composite.metadata) {
      throw new Error(
        "failed to update resource, invalid resource data",
        dataString,
      );
    }
  ...
  async deleteResourceTag(resourceId: string, tagName: string) {
    ...
    await this.backend.js__store_remove_resource_tag_by_name(
      resourceId,
      tagName,
    );
  }
  ...
  async listResourceIDsByTags(
    ...
    if (excludeWithinSpaces) {
      raw =
        await this.backend.js__store_list_resources_by_tags_no_space(tagsData);
    } else {
  ...
  async listAllResourcesAndSpaces(tags: SFFSResourceTag[]) {
    ...
    const raw =
      await this.backend.js__store_list_all_resources_and_spaces(tagsData);
  ...
  async searchChatResourcesAI(
    ...
    this.log.debug(
      "searching resources with AI query",
      query,
      "model:",
      model,
      "opts:",
    ...
    const compositeResources =
      this.parseData<SFFSRawCompositeResource[]>(rawResponse);
    if (!compositeResources) {
      return [];
    }

    return compositeResources.map((resource) =>
      this.convertCompositeResourceToResource(resource),
    );
  }
  ...
  async addItemsToSpace(
    ...
    this.log.debug(
      "creating space entries for space",
      space_id,
      "entries:",
      typedItems,
    );
  ...
  async addSubspacesToSpace(
    ...
    this.log.debug(
      "creating sub space entries for space",
      space_id,
      "entries:",
      typedItems,
    );
  ...
  async readDataFile(resourceId: string): Promise<Uint8Array> {
    ...
    const uInt8 = (await this.fs.readResource(
      resourceId,
  ...
  async createHistoryEntry(entry: HistoryEntry): Promise<HistoryEntry> {
    ...
    const newRawEntry =
      await this.backend.js__store_create_history_entry(stringEntry);
    const newEntry = this.parseData<SFFSRawHistoryEntry>(newRawEntry);
    if (!newEntry) {
      throw new Error(
        "failed to create history entry, invalid data",
        newRawEntry,
      );
    }
  ...
  async getHistoryEntries(limit?: number): Promise<HistoryEntry[]> {
    ...
    const rawEntries =
      await this.backend.js__store_get_all_history_entries(limit);
  ...
  async updateHistoryEntry(data: Partial<HistoryEntry>): Promise<void> {
    ...
    const rawEntry = this.convertHistoryEntryToRawHistoryEntry(
  ...
  async importBrowserHistory(type: BrowserType) {
    ...
    const rawEntries =
      await this.backend.js__store_import_browser_history(type);
    const entries = this.parseData<SFFSRawHistoryEntry[]>(rawEntries);
    if (!entries) {
      return [];
    }
  ...
  async importBrowserBookmarks(type: BrowserType) {
    ...
    const rawEntries =
      await this.backend.js__store_import_browser_bookmarks(type);
  ...
  async searchHistoryEntriesByHostnamePrefix(
    ...
    const rawEntries =
      await this.backend.js__store_search_history_entries_by_hostname_prefix(
        prefix,
        since,
      );
    const entries = this.parseData<SFFSRawHistoryEntry[]>(rawEntries);
  ...
  async searchHistoryEntriesByHostname(url: string): Promise<HistoryEntry[]> {
    ...
    const rawEntries =
      await this.backend.js__store_search_history_entries_by_hostname(url);
    const entries = this.parseData<SFFSRawHistoryEntry[]>(rawEntries);
    if (!entries) {
      return [];
    }
  ...
  async searchHistoryEntriesByUrlAndTitle(
    query: string,
    since?: Date,
  ): Promise<HistoryEntry[]> {
    this.log.debug("searching history entries by url and title for", query);
    const rawEntries =
      await this.backend.js__store_search_history_entries_by_url_and_title(
        query,
        since,
      );
    const entries = this.parseData<SFFSRawHistoryEntry[]>(rawEntries);
  ...
  async createAIChat(
    ...
    this.log.debug(
      `creating ai chat "${title}" with system prompt: ${system_prompt}`,
    );
    return this.parseData<string>(
      await this.backend.js__store_create_ai_chat(
        system_prompt ?? "",
        title ?? "",
      ),
    );
  ...
  async getAIDocsSimilarity(
    ...
    this.log.debug(
      "getting ai docs similarity with query",
      query,
      "threshold",
      threshold,
    );
    const raw = await this.backend.js__ai_get_docs_similarity(
      query,
      docs,
      threshold,
    );
  ...
  async getResourcesViaPrompt(
    ...
    if (!response) {
      throw new Error(
        `failed to query SFFS resources, invalid response data: ${rawResponse}`,
      );
    }
  ...
  async storeAIApp(
    ...
    return this.backend.js__store_create_app(
      appType,
      content,
      name,
      icon,
