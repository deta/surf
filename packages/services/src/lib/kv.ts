...
export class KVStore<T extends BaseKVItem> {
  ...
  async create(
    data: Optional<T, "id" | "createdAt" | "updatedAt">,
  ): Promise<T> {
  ...
  async all(): Promise<T[]> {
    try {
      const items = optimisticParseJSON<T[]>(
        await this.backend.js__kv_list(this.tableName),
      );
      if (!items) {
        return [];
      }
      ...
      this.log.error(
        `Failed to get all items from "${this.tableName}":`,
        error,
      );
      throw error;
    }
  ...
  async read(id: string): Promise<T | undefined> {
    try {
      ...
      this.log.error(
        `Failed to read item with ID ${id} from "${this.tableName}":`,
        error,
      );
      throw error;
    }
  ...
  async update(id: string, updatedItem: Partial<T>): Promise<number> {
    try {
      ...
      await this.backend.js__kv_update(
        this.tableName,
        id,
      ...
      this.log.error(
        `Failed to update item with ID ${id} in "${this.tableName}":`,
        error,
      );
      throw error;
    }
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      this.log.error(
        `Failed to delete item with ID ${id} from "${this.tableName}":`,
        error,
      );
      throw error;
    }
  ...
  async bulkUpdate(
    items: { id: string; updates: Partial<T> }[],
  ): Promise<void> {
    const datetime = new Date().toISOString();
    ...
    try {
      ...
      this.log.debug(
        `Bulk updated ${items.length} items in "${this.tableName}"`,
      );
    } catch (error) {
      this.log.error(
        `Failed to bulk update items in "${this.tableName}":`,
        error,
      );
      throw error;
    }
